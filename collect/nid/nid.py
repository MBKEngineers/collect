...
def get_nid_daily_data(sites, water_year):
    """
    Returns a `dict` and creates JSON file of the data and info for NID sites provided

    Arguments:
        sites (list of str): list of NID sites to pull from
        water_year (int): current water year
    Returns:
        json_data (dict): dictionary of data and info for each site
    """
    json_data = {}
    json_data['info'] = []
    ...
    for site in sites:
        ...
        for month in months:
            ...
            month_df['date'] = data['Day'].apply(lambda x: str(x) + month + str(water_year))
...
if __name__ == '__main__':
    ...
    data = get_nid_daily_data(sites, 2021)
